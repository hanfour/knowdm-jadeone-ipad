...
const FrenchAestheticsPage: React.FC = () => {
  ...
  const prevAward = () => {
    setAwardsModal(prev => ({
      ...prev,
      currentIndex: (prev.currentIndex - 1 + awardImages.length) % awardImages.length,
    }));
  };
  ...
  const nextAward = () => {
    setAwardsModal(prev => ({
      ...prev,
      currentIndex: (prev.currentIndex + 1) % awardImages.length,
    }));
  };
    ...
    <div className="relative w-full h-full overflow-hidden bg-black">
      ...
      {awardsModal.isOpen && (
        ...
        >
          ...
          >
            {awardImages.map((image, index) => (
              <img
                ...
                decoding="async"
                className={`absolute inset-0 w-full h-full object-contain transition-opacity duration-500 ${
                  index === awardsModal.currentIndex ? 'opacity-100' : 'opacity-0'
                }`}
              />
            ))}
          </div>

          <CloseButton onClick={(e) => { e.stopPropagation(); closeAwardsModal(); }} />

          {/* 上一張按鈕 */}
          <button
            onClick={(e) => { e.stopPropagation(); prevAward(); }}
            className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/20 text-white flex items-center justify-center hover:bg-white/30 transition-colors"
            style={{ zIndex: 10 }}
            aria-label="上一張"
          >
          ...
          {/* 下一張按鈕 */}
          <button
            onClick={(e) => { e.stopPropagation(); nextAward(); }}
            className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/20 text-white flex items-center justify-center hover:bg-white/30 transition-colors"
            style={{ zIndex: 10 }}
            aria-label="下一張"
          >
