...
export interface AnimationConfig {
  ...
  phase1?: boolean;  // 第二幕：第二段文字
  phase3?: boolean;  // 主標題 + 副標題 + Shine 掃光
  phase6?: boolean;  // Logo 階段
}
...
// 背景組件
const BackgroundLayer: React.FC<{ type: 'green' | 'pattern'; backgroundImage?: string }> = React.memo(({ type, backgroundImage }) => (
  <>
    {type === 'green' ? (
      <>
...
const IntroAnimation: React.FC<IntroAnimationProps> = ({
  ...
  introText1 = '聽樹先生唱歌，看花小姐飛舞。',
  introText2 = '公園在宅休閒　都會輕旅建築。',
  mainTitle = '高訂品味 對味不凡',
  ...
  const getInitialPhase = () => {
    if (config.phase0) return 0;
    if (config.phase1) return 1;
    if (config.phase3) return 3;
    if (config.phase6) return 6;
    return -1;
  };
  ...
  const getNextPhase = (currentPhase: number): number => {
    if (currentPhase === 0 && config.phase1) return 1;
    if (currentPhase <= 1 && config.phase3) return 3;
    if (currentPhase <= 3 && config.phase6) return 6;
    return -1;
  };
  ...
  if (phase === 3) {
    return (
      <div
        ...
        onClick={skipIntro}
      >
        <BackgroundLayer type="pattern" backgroundImage={backgroundImage} />
        <div
          className={`absolute inset-0 flex items-center justify-center transition-opacity duration-700 ${fadeOut ? 'opacity-0' : 'opacity-100'}`}
        >
